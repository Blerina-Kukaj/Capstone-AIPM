        ...
        "findings": [
            {
                ...
                "evidence": [{"source_id": source_id, "source_type": "ticket", "excerpt": "upstream test excerpt"}],
                "tags": ["mock"],
                "metadata": {},
            }
        ],
        ...
        "findings": [
            {
                ...
                "metadata": {
                    ...
                    "acceptance_criteria": (
                        "GIVEN a notification arrives WHEN ML priority score < 0.3 THEN the notification is suppressed"
                    ),
                    "priority": "P0",
                    "complexity": "M",
                    "phase": "MVP",
...
async def test_customer_evidence_linking(run_config, default_policy, sample_context):
    ...
    for f in output.findings:
        ...
        for ev in f.evidence:
            assert ev.source_id in valid_ids, f"Finding {f.id} references unknown source_id '{ev.source_id}'"
...
async def test_risk_agent_policy_gate(run_config, strict_policy, sample_context):
    ...
    gate_path = Path(run_config.output_dir) / run_config.run_id / "findings" / "risk_gate_result.json"
...
async def test_lead_pm_conflict_resolution(run_config, default_policy, sample_context):
    ...
    output_req = AgentOutput(
        ...
        run_id=run_config.run_id,
        findings=[finding_req],
    )
    output_risk = AgentOutput(
        agent_id="risk",
        agent_name="Risk Agent",
        run_id=run_config.run_id,
        findings=[finding_risk],
    )
